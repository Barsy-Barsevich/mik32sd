#include "sd.h"

static void SPI0_Init(void)
{
    hspi0.Instance = SPI_0;

    /* Режим SPI */
    hspi0.Init.SPI_Mode = HAL_SPI_MODE_MASTER;

    /* Настройки */
    hspi0.Init.CLKPhase = SPI_PHASE_ON;
    hspi0.Init.CLKPolarity = SPI_POLARITY_HIGH;
    hspi0.Init.ThresholdTX = 4;

    /* Настройки для ведущего */
    hspi0.Init.BaudRateDiv = SPI_BAUDRATE_DIV256;
    hspi0.Init.Decoder = SPI_DECODER_NONE;
    hspi0.Init.ManualCS = SPI_MANUALCS_ON;
    hspi0.Init.ChipSelect = SPI_CS_0;

    if (HAL_SPI_Init(&hspi0) != HAL_OK)
    {
        xprintf("SPI_Init_Error\n");
    }
}


void SD_SendCommand(SD_Commands_enum command, uint32_t operand, uint8_t crc, uint8_t* resp)
{
    uint8_t data = 0xFF;
    //HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);
    do
    {
        HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    } while (*resp != 0xFF);
    data = (uint8_t)command;
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = ((operand>>24) & 0xFF);
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = ((operand>>16) & 0xFF);
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = ((operand>>8) & 0xFF);
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = (operand & 0xFF);
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = crc;
    HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    // data = 0xFF;
    // HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    // data = 0xFF;
    // HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    data = 0xFF;
    do
    {
        HAL_SPI_Exchange(&hspi0, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
    } while (*resp == 0xFF);
    
    /* R7 response */
    if ((command == CMD8) || (command == CMD58))
    {
        for (uint8_t i=0; i<4; i++)
            HAL_SPI_Exchange(&hspi0, &data, resp+i+1, 1, SPI_TIMEOUT_DEFAULT);
    }

    //HAL_SPI_CS_Disable(&hspi0);
    xprintf("Com: 0x%X; R1: %08b", command, resp[0]);
    if ((command == CMD8) || (command == CMD58))
    {
        uint32_t ocr = ((uint32_t)resp[1]<<24 | (uint32_t)resp[2]<<16 |
                        (uint32_t)resp[3]<<8 | resp[4]);
        xprintf(", extra: %032b", ocr);
    }
    xprintf("\n");
}




SD_Status_enum SD_Init(SD_Descriptor_t* param)
{
    SPI0_Init();
    HAL_SPI_CS_Disable(&hspi0);
    HAL_DelayMs(100);
    //80 тактов на линии SCK
    uint8_t data = 0xFF;
    uint8_t dummy;
    for (uint8_t i=0; i<10; i++)
        HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);

    uint8_t resp[5];

    HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);

    SD_SendCommand(CMD0, 0, 0x95, resp);

    SD_SendCommand(CMD8, 0x1AA, 0x87, resp);
    
    /* It is v1 SD-card or not-SD-card */
    if (resp[0] & SD_R1_ILLEGAL_COMMAND_M)
    {
        SD_SendCommand(CMD58, 0, 0xFF, resp);
        uint32_t ocr = ((uint32_t)resp[1]<<24 | (uint32_t)resp[2]<<16 |
                        (uint32_t)resp[3]<<8 | resp[4]);
        if (!(param->voltage & ocr))
        {
            HAL_SPI_CS_Disable(&hspi0);
            return SD_IncorrectVoltage;
        }
        if (resp[0] & SD_R1_ILLEGAL_COMMAND_M)
        {
            HAL_SPI_CS_Disable(&hspi0);
            return SD_UnknownCard;
        }
        uint8_t counter = 200;

        /* Trying to send ACMD41 */
        SD_SendCommand(CMD55, 0, 0xFF, resp);
        SD_SendCommand(ACMD41, 0x40000000, 0xFF, resp);

        /* It is a MMC */
        if (resp[0] & SD_R1_ILLEGAL_COMMAND_M)
        {
            /* >74 clock cycles on SCK */
            for (uint8_t i=0; i<10; i++) 
                HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
            /* Go from idle_mode */
            while (resp[0] & SD_R1_IDLE_STATE_M)
            {
                SD_SendCommand(CMD1, 0, 0xFF, resp);
                counter -= 1;
                if (counter == 0)
                {
                    HAL_SPI_CS_Disable(&hspi0);
                    return SD_TimeoutError;
                }
            }
            param->type = MMC;
            HAL_SPI_CS_Disable(&hspi0);
            return SD_OK;
        }
        /* It is a SDv1 */
        else
        {
            /* Go from idle_mode */
            while (resp[0] & SD_R1_IDLE_STATE_M)
            {
                SD_SendCommand(CMD55, 0, 0xFF, resp);
                SD_SendCommand(ACMD41, 0x40000000, 0xFF, resp);
                counter -= 1;
                if (counter == 0)
                {
                    HAL_SPI_CS_Disable(&hspi0);
                    return SD_TimeoutError;
                }
            }
            param->type = SDv1;
            HAL_SPI_CS_Disable(&hspi0);
            return SD_OK;
        }
    }

    /* It is SD v2, SDHC or SDXC card */
    else
    {
        /* check the check_pattern */
        if (resp[4] != 0xAA)
        {
            HAL_SPI_CS_Disable(&hspi0);
            return SD_CommunicationError;
        }

        /* Check the card's valid voltage */
        SD_SendCommand(CMD58, 0, 0xFF, resp);
        uint32_t ocr = ((uint32_t)resp[1]<<24 | (uint32_t)resp[2]<<16 |
                        (uint32_t)resp[3]<<8 | resp[4]);
        if (!(param->voltage & ocr))
        {
            HAL_SPI_CS_Disable(&hspi0);
            return SD_IncorrectVoltage;
        }

        /* >74 clock cycles on SCK */
            for (uint8_t i=0; i<10; i++) 
                HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);

        /* Go from idle_mode */
        uint8_t counter = 200;
        while (resp[0] & SD_R1_IDLE_STATE_M)
        {
            SD_SendCommand(CMD55, 0, 0xFF, resp);
            SD_SendCommand(ACMD41, 0x40000000, 0xFF, resp);
            counter -= 1;
            if (counter == 0)
            {
                HAL_SPI_CS_Disable(&hspi0);
                return SD_TimeoutError;
            }
        }

        /* Read the CCS value */
        SD_SendCommand(CMD58, 0, 0xFF, resp);
        uint8_t ccs = resp[1] & 0b01000000;
        if (ccs == 0) param->type = SDv2;
        else param->type = SDHC;
        HAL_SPI_CS_Disable(&hspi0);
        return SD_OK;
    }
    
    // while(1)
    // {
    //     SD_SendCommand(CMD55, 0, 0xFF);
    //     SD_SendCommand(ACMD41, 0x40000000, 0xFF);
    // }
    //     HAL_DelayMs(100);

    // while (1)
    // {
    // // HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);

    // // data = 0xFF;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x40;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x00;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x00;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x00;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x00;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0x95;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0xFF;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // data = 0xFF;
    // // HAL_SPI_Exchange(&hspi0, &data, &dummy, 1, SPI_TIMEOUT_DEFAULT);
    // // HAL_SPI_CS_Disable(&hspi0);
    // //     xprintf("R1: %08b\n", dummy);
    // //     HAL_DelayMs(100);
    // }
}


SD_Status_enum SD_SingleRead(SD_Descriptor_t* local, uint32_t addr, uint8_t* buf)
{
    uint8_t resp, dummy = 0xFF;
    HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);
    SD_SendCommand(CMD17, addr, 0xff, &resp);
    // for (uint16_t i=0; i<512; i++)
    //     HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    if (resp != 0) return resp;
    uint16_t counter = 0;
    while ((resp != 0xFE) && (resp != 0xFC))
    {
        HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
        if (counter >= SD_SREAD_WAIT_ATTEMPTS) return SD_TimeoutError;
        counter += 1;
        if (resp == 0xFF) continue;
    }
    for (uint16_t i=0; i<512; i++)
    {
        HAL_SPI_Exchange(&hspi0, &dummy, buf+i, 1, SPI_TIMEOUT_DEFAULT);
    }
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    HAL_SPI_CS_Disable(&hspi0);
    return SD_OK;
}


SD_Status_enum SD_SingleWrite(SD_Descriptor_t* local, uint32_t addr, uint8_t* buf)
{
    uint8_t resp, dummy = 0xFF;
    HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);
    SD_SendCommand(CMD24, addr, 0xff, &resp);
    if (resp != 0) return resp;
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    dummy = 0xFC;
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    for (uint16_t i=0; i<512; i++)
    {
        HAL_SPI_Exchange(&hspi0, buf+i, &resp, 1, SPI_TIMEOUT_DEFAULT);
    }
    dummy = 0xFF;
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    HAL_SPI_Exchange(&hspi0, &dummy, &resp, 1, SPI_TIMEOUT_DEFAULT);
    return SD_OK;
}


SD_Status_enum SD_SingleErase(SD_Descriptor_t* local, uint32_t addr)
{
    uint8_t resp, dummy = 0xFF;
    HAL_SPI_CS_Enable(&hspi0, SPI_CS_0);
    SD_SendCommand(CMD32, addr, 0xFF, &resp);
    if (resp != 0) return resp;
    SD_SendCommand(CMD33, addr, 0xFF, &resp);
    if (resp != 0) return resp;
    SD_SendCommand(CMD38, 0, 0xFF, &resp);
}